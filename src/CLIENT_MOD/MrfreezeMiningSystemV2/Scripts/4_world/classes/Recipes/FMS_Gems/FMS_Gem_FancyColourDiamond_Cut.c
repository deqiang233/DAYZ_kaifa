class CraftFancyColourDiamond extends RecipeBase
{	
	static bool ENABLE_MINIGAME = true;

	override void Init()
	{
		m_Name = "Craft Fancy Colour Diamond Gem";
		...
		InsertIngredient(0, "FreezeGem_FancyColourDiamond");
		...
		AddResult("FreezeGem_FancyColourDiamond_Cut");
	...
	{
		if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftFancyColourDiamond"))
		{
			int seconds = g_FreezeCraftingTimes.Get("CraftFancyColourDiamond");
		...
		else if (GetGame().IsServer())
		{
			...
			if (craftConfig)
			{
				m_AnimationLength = craftConfig.GetAnimationLength("CraftFancyColourDiamond");
			}
		}
	...
	{		// Handle both Static and regular grindstones
		...
		if (grindingStone && GetGame().IsServer())
		{
			...
			if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftFancyColourDiamond"))
			{
				configSeconds = g_FreezeCraftingTimes.Get("CraftFancyColourDiamond");
			}
			else
			{
				FreezeCraftingTimesConfig craftConfig = FreezeCraftingTimesConfig.GetCached();
				if (craftConfig)
				{
					configSeconds = craftConfig.GetCraftTimeSeconds("CraftFancyColourDiamond");
				}
			}
			
			float actualCraftTime = m_AnimationLength * 4;
			LoggingUtility.LogDebug("[GrindingStone] CraftFancyColourDiamond - Config: " + configSeconds + "s | Actual: " + actualCraftTime + "s | AnimLength: " + m_AnimationLength + " | Player: " + player.GetIdentity().GetName());
		}
		...
		if (!ENABLE_MINIGAME)
		{
			gemResult = "FreezeGem_FancyColourDiamond_Cut";
		}
		else
		{
			...
			if (roll <= 0.50)
			{
				gemResult = "FreezeGem_FancyColourDiamond_Flawed";
			}
			else if (roll <= 0.85)
			{
				gemResult = "FreezeGem_FancyColourDiamond_Standard";
			}
			else
			{
				gemResult = "FreezeGem_FancyColourDiamond_Perfect";
			}
		}
