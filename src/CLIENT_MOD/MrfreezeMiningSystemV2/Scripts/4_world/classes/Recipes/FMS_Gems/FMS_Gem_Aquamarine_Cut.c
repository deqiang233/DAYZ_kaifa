class CraftAquamarine extends RecipeBase
{	
	static bool ENABLE_MINIGAME = true;

	override void Init()
	{
		m_Name = "Craft Aquamarine Gem";
		...
		InsertIngredient(0, "FreezeGem_Aquamarine");
		...
		AddResult("FreezeGem_Aquamarine_Cut");
	...
	{
		if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftAquamarine"))
		{
			int seconds = g_FreezeCraftingTimes.Get("CraftAquamarine");
		...
		else if (GetGame().IsServer())
		{
			...
			if (craftConfig)
			{
				m_AnimationLength = craftConfig.GetAnimationLength("CraftAquamarine");
			}
		}
	...
	{		// Handle both Static and regular grindstones
		...
		if (grindingStone && GetGame().IsServer())
		{
			...
			if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftAquamarine"))
			{
				configSeconds = g_FreezeCraftingTimes.Get("CraftAquamarine");
			}
			else
			{
				FreezeCraftingTimesConfig craftConfig = FreezeCraftingTimesConfig.GetCached();
				if (craftConfig)
				{
					configSeconds = craftConfig.GetCraftTimeSeconds("CraftAquamarine");
				}
			}
			
			float actualCraftTime = craftDurationSeconds;
			LoggingUtility.LogDebug("[GrindingStone] CraftAquamarine - Config: " + configSeconds + "s | Actual: " + actualCraftTime + "s | AnimLength: " + m_AnimationLength + " | Player: " + player.GetIdentity().GetName());
		}
		...
		if (!ENABLE_MINIGAME)
		{
			gemResult = "FreezeGem_Aquamarine_Cut";
		}
		else
		{
			...
			if (roll <= 0.50)
			{
				gemResult = "FreezeGem_Aquamarine_Flawed";
			}
			else if (roll <= 0.85)
			{
				gemResult = "FreezeGem_Aquamarine_Standard";
			}
			else
			{
				gemResult = "FreezeGem_Aquamarine_Perfect";
			}
		}
