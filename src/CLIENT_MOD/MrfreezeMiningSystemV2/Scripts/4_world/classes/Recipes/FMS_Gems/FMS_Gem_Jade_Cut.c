class CraftJade extends RecipeBase
{	
	static bool ENABLE_MINIGAME = true;

	override void Init()
	{
		m_Name = "Craft Jade Gem";
		...
		InsertIngredient(0, "FreezeGem_Jade");
	...
	{
		if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftJade"))
		{
			int seconds = g_FreezeCraftingTimes.Get("CraftJade");
		...
		else if (GetGame().IsServer())
		{
			...
			if (craftConfig)
			{
				m_AnimationLength = craftConfig.GetAnimationLength("CraftJade");
			}
		}
	...
	{		// Handle both Static and regular grindstones
		...
		if (grindingStone && GetGame().IsServer())
		{
			...
			if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftJade"))
			{
				configSeconds = g_FreezeCraftingTimes.Get("CraftJade");
			}
			else
			{
				FreezeCraftingTimesConfig craftConfig = FreezeCraftingTimesConfig.GetCached();
				if (craftConfig)
				{
					configSeconds = craftConfig.GetCraftTimeSeconds("CraftJade");
				}
			}
			
			float actualCraftTime = m_AnimationLength * 4;
			LoggingUtility.LogDebug("[GrindingStone] CraftJade - Config: " + configSeconds + "s | Actual: " + actualCraftTime + "s | AnimLength: " + m_AnimationLength + " | Player: " + player.GetIdentity().GetName());
		}
		...
		else
		{
			...
			if (roll <= 0.50)
			{
				gemResult = "FreezeGem_Jade_Flawed";
			}
			else if (roll <= 0.85)
			{
				gemResult = "FreezeGem_Jade_Standard";
			}
			else
			{
				gemResult = "FreezeGem_Jade_Perfect";
			}
		}
