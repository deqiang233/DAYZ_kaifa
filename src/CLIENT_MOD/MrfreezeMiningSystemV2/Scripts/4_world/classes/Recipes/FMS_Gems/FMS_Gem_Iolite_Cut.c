class CraftIolite extends RecipeBase
{	
	static bool ENABLE_MINIGAME = true;

	override void Init()
	{
		m_Name = "Craft Iolite Gem";
		...
		InsertIngredient(0, "FreezeGem_Iolite");
		...
		AddResult("FreezeGem_Iolite_Cut");
	...
	{
		if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftIolite"))
		{
			int seconds = g_FreezeCraftingTimes.Get("CraftIolite");
		...
		else if (GetGame().IsServer())
		{
			...
			if (craftConfig)
			{
				m_AnimationLength = craftConfig.GetAnimationLength("CraftIolite");
			}
		}
	...
	{		// Handle both Static and regular grindstones
		...
		if (grindingStone && GetGame().IsServer())
		{
			...
			if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftIolite"))
			{
				configSeconds = g_FreezeCraftingTimes.Get("CraftIolite");
			}
			else
			{
				FreezeCraftingTimesConfig craftConfig = FreezeCraftingTimesConfig.GetCached();
				if (craftConfig)
				{
					configSeconds = craftConfig.GetCraftTimeSeconds("CraftIolite");
				}
			}
			
			float actualCraftTime = m_AnimationLength * 4;
			LoggingUtility.LogDebug("[GrindingStone] CraftIolite - Config: " + configSeconds + "s | Actual: " + actualCraftTime + "s | AnimLength: " + m_AnimationLength + " | Player: " + player.GetIdentity().GetName());
		}
		...
		if (!ENABLE_MINIGAME)
		{
			gemResult = "FreezeGem_Iolite_Cut";
		}
		else
		{
			...
			if (roll <= 0.50)
			{
				gemResult = "FreezeGem_Iolite_Flawed";
			}
			else if (roll <= 0.85)
			{
				gemResult = "FreezeGem_Iolite_Standard";
			}
			else
			{
				gemResult = "FreezeGem_Iolite_Perfect";
			}
		}
