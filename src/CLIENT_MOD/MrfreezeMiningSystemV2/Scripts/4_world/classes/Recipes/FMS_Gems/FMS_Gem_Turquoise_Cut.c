class CraftTurquoise extends RecipeBase
{	
	static bool ENABLE_MINIGAME = true;

	override void Init()
	{
		m_Name = "Craft Turquoise Gem";
		...
		InsertIngredient(0, "FreezeGem_Turquoise");
		...
		AddResult("FreezeGem_Turquoise_Cut");
	...
	{
		if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftTurquoise"))
		{
			int seconds = g_FreezeCraftingTimes.Get("CraftTurquoise");
		...
		else if (GetGame().IsServer())
		{
			...
			if (craftConfig)
			{
				m_AnimationLength = craftConfig.GetAnimationLength("CraftTurquoise");
			}
		}
	...
	{		// Handle both Static and regular grindstones
		...
		if (grindingStone && GetGame().IsServer())
		{
			...
			if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftTurquoise"))
			{
				configSeconds = g_FreezeCraftingTimes.Get("CraftTurquoise");
			}
			else
			{
				FreezeCraftingTimesConfig craftConfig = FreezeCraftingTimesConfig.GetCached();
				if (craftConfig)
				{
					configSeconds = craftConfig.GetCraftTimeSeconds("CraftTurquoise");
				}
			}
			
			float actualCraftTime = m_AnimationLength * 4;
			LoggingUtility.LogDebug("[GrindingStone] CraftTurquoise - Config: " + configSeconds + "s | Actual: " + actualCraftTime + "s | AnimLength: " + m_AnimationLength + " | Player: " + player.GetIdentity().GetName());
		}
		...
		if (!ENABLE_MINIGAME)
		{
			gemResult = "FreezeGem_Turquoise_Cut";
		}
		else
		{
			...
			if (roll <= 0.50)
			{
				gemResult = "FreezeGem_Turquoise_Flawed";
			}
			else if (roll <= 0.85)
			{
				gemResult = "FreezeGem_Turquoise_Standard";
			}
			else
			{
				gemResult = "FreezeGem_Turquoise_Perfect";
			}
		}
