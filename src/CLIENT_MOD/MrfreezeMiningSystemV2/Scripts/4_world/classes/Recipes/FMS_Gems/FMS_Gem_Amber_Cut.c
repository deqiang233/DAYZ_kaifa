class CraftAmber extends RecipeBase
{	
	static bool ENABLE_MINIGAME = true;

	override void Init()
	{
		m_Name = "Craft Amber Gem";
		...
		InsertIngredient(0, "FreezeGem_Amber");
	...
	{
		if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftAmber"))
		{
			int seconds = g_FreezeCraftingTimes.Get("CraftAmber");
		...
		else if (GetGame().IsServer())
		{
			...
			if (craftConfig)
			{
				m_AnimationLength = craftConfig.GetAnimationLength("CraftAmber");
			}
		}
	...
	override void Do(ItemBase ingredients[], PlayerBase player, array<ItemBase> results, float specialty_weight)
	{
		...
		if (grindingStone && GetGame().IsServer())
		{
			...
			if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftAmber"))
			{
				configSeconds = g_FreezeCraftingTimes.Get("CraftAmber");
			}
			else
			{
				FreezeCraftingTimesConfig craftConfig = FreezeCraftingTimesConfig.GetCached();
				if (craftConfig)
				{
					configSeconds = craftConfig.GetCraftTimeSeconds("CraftAmber");
				}
			}
			
			float actualCraftTime = craftDurationSeconds;
			LoggingUtility.LogDebug("[GrindingStone] CraftAmber - Config: " + configSeconds + "s | Actual: " + actualCraftTime + "s | AnimLength: " + m_AnimationLength + " | Player: " + player.GetIdentity().GetName());
		}
		...
		else
		{
			...
			if (roll <= 0.50)
			{
				gemResult = "FreezeGem_Amber_Flawed";
			}
			else if (roll <= 0.85)
			{
				gemResult = "FreezeGem_Amber_Standard";
			}
			else
			{
				gemResult = "FreezeGem_Amber_Perfect";
			}
		}
