class CraftSulphurPowder extends RecipeBase
{	
	
	override void Init()
	{
		m_Name = "Crush Sulphur";
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0, "FreezeRef_Sulphur");//you can insert multiple ingredients this way
		...
		//result1
		AddResult("FreezePowder_Sulphur");//add results here
    ...
    {
        if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftSulphurPowder"))
        {
            int seconds = g_FreezeCraftingTimes.Get("CraftSulphurPowder");
        ...
        else if (GetGame().IsServer())
        {
            ...
            if (craftConfig)
            {
                m_AnimationLength = craftConfig.GetAnimationLength("CraftSulphurPowder");
            }
        }
        ...
        if (anvil)
        {
            ...
            return true; // Anvil exists and has a valid hammer
        }
        return false; // No anvil present
    }
	...
	override void Do(ItemBase ingredients[], PlayerBase player, array<ItemBase> results, float specialty_weight) // gets called upon recipe's completion
	{
		...
		if (anvil && GetGame().IsServer())
		{
			...
			if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftSulphurPowder"))
			{
				configSeconds = g_FreezeCraftingTimes.Get("CraftSulphurPowder");
			}
			else
			{
				FreezeCraftingTimesConfig craftConfig = FreezeCraftingTimesConfig.GetCached();
				if (craftConfig)
				{
					configSeconds = craftConfig.GetCraftTimeSeconds("CraftSulphurPowder");
				}
			}
			
			float actualCraftTime = m_AnimationLength * 4;
			LoggingUtility.LogDebug("[Anvil] CraftSulphurPowder - Config: " + configSeconds + "s | Actual: " + actualCraftTime + "s | AnimLength: " + m_AnimationLength + " | Player: " + player.GetIdentity().GetName());
		}
