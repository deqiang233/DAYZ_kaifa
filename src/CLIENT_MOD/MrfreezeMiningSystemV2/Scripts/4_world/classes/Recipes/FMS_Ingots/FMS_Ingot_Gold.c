class CraftGoldIngot extends RecipeBase
{	

	override void Init()
	{
		m_Name = "Craft Gold Ingot";
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0, "FreezeNugget_gold");//you can insert multiple ingredients this way
		...
		//result1
		AddResult("FreezeBar_gold");//add results here
    ...
    override bool CanDo(ItemBase ingredients[], PlayerBase player) // final check for recipe's validity
    {
		...
		if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftGoldIngot"))
        {
            int seconds = g_FreezeCraftingTimes.Get("CraftGoldIngot");
            m_AnimationLength = seconds * 0.25;
        }
        else if (GetGame().IsServer())
        {
            // Server: use plugin directly
			FreezeCraftingTimesConfig craftConfig = FreezeCraftingTimesConfig.GetCached();
            if (craftConfig)
            {
                m_AnimationLength = craftConfig.GetAnimationLength("CraftGoldIngot");
            }
        }
    ...
    override void Do(ItemBase ingredients[], PlayerBase player, array<ItemBase> results, float specialty_weight) // gets called upon recipe's completion
    {
		...
		if (anvil && GetGame().IsServer())
		{
			...
			if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftGoldIngot"))
			{
				configSeconds = g_FreezeCraftingTimes.Get("CraftGoldIngot");
			}
			else
			{
				FreezeCraftingTimesConfig craftConfig = FreezeCraftingTimesConfig.GetCached();
				if (craftConfig)
				{
					configSeconds = craftConfig.GetCraftTimeSeconds("CraftGoldIngot");
				}
			}
			
			float actualCraftTime = m_AnimationLength * 4;
			LoggingUtility.LogDebug("[Anvil] CraftGoldIngot - Config: " + configSeconds + "s | Actual: " + actualCraftTime + "s | AnimLength: " + m_AnimationLength + " | Player: " + player.GetIdentity().GetName());
		}
