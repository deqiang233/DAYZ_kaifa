class CraftUraniumIngot extends RecipeBase
{	
	
	override void Init()
	{
		m_Name = "Craft Uranium Ingot";
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0, "FreezeNugget_uranium");//you can insert multiple ingredients this way
		...
		//result1
		AddResult("FreezeBar_uranium");//add results here
    ...
    {
        if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftUraniumIngot"))
        {
            int seconds = g_FreezeCraftingTimes.Get("CraftUraniumIngot");
        ...
        else if (GetGame().IsServer())
        {
            ...
            if (craftConfig)
            {
                m_AnimationLength = craftConfig.GetAnimationLength("CraftUraniumIngot");
            }
        }
    ...
    {        // Handle both Static and regular anvils
		...
		if (anvil && GetGame().IsServer())
		{
			...
			if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftUraniumIngot"))
			{
				configSeconds = g_FreezeCraftingTimes.Get("CraftUraniumIngot");
			}
			else
			{
				FreezeCraftingTimesConfig craftConfig = FreezeCraftingTimesConfig.GetCached();
				if (craftConfig)
				{
					configSeconds = craftConfig.GetCraftTimeSeconds("CraftUraniumIngot");
				}
			}
			
			float actualCraftTime = m_AnimationLength * 4;
			LoggingUtility.LogDebug("[Anvil] CraftUraniumIngot - Config: " + configSeconds + "s | Actual: " + actualCraftTime + "s | AnimLength: " + m_AnimationLength + " | Player: " + player.GetIdentity().GetName());
		}
