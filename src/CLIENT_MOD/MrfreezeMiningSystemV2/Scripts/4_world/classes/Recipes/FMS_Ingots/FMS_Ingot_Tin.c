class CraftTinIngot extends RecipeBase
{	
	
	override void Init()
	{
		m_Name = "Craft Tin Ingot";
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0, "FreezeNugget_tin");//you can insert multiple ingredients this way
		...
		//result1
		AddResult("FreezeBar_tin");//add results here
    ...
    {
        if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftTinIngot"))
        {
            int seconds = g_FreezeCraftingTimes.Get("CraftTinIngot");
        ...
        else if (GetGame().IsServer())
        {
            ...
            if (craftConfig)
            {
                m_AnimationLength = craftConfig.GetAnimationLength("CraftTinIngot");
            }
        }
    ...
    {        // Handle both Static and regular anvils
		...
		if (anvil && GetGame().IsServer())
		{
			...
			if (g_FreezeCraftingTimes && g_FreezeCraftingTimes.Contains("CraftTinIngot"))
			{
				configSeconds = g_FreezeCraftingTimes.Get("CraftTinIngot");
			}
			else
			{
				FreezeCraftingTimesConfig craftConfig = FreezeCraftingTimesConfig.GetCached();
				if (craftConfig)
				{
					configSeconds = craftConfig.GetCraftTimeSeconds("CraftTinIngot");
				}
			}
			
			float actualCraftTime = m_AnimationLength * 4;
			LoggingUtility.LogDebug("[Anvil] CraftTinIngot - Config: " + configSeconds + "s | Actual: " + actualCraftTime + "s | AnimLength: " + m_AnimationLength + " | Player: " + player.GetIdentity().GetName());
		}
