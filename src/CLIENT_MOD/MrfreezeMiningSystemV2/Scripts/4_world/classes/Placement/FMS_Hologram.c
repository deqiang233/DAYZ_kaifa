...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(FreezeAnvil) )
				{
					FreezeAnvil Freezeanvil = FreezeAnvil.Cast(obj_collided);
					return Freezeanvil;
				}
				if( obj_collided.IsInherited(FreezeGrindingStone) )
				{
					FreezeGrindingStone FreezegrindingStone = FreezeGrindingStone.Cast(obj_collided);
					return FreezegrindingStone;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands)
		{
			if (item_in_hands.IsInherited(FreezeAnvil_Kit))
				return "FreezeAnvil";
			
			if (item_in_hands.IsInherited(FreezeFurnace_Kit))
				return "FreezeFurnace";
			
			if (item_in_hands.IsInherited(FreezeGrindingStone_Kit))
				return "FreezeGrindingStone";
		}
	...
	override void EvaluateCollision(ItemBase action_item = null)
	{
		...
		if (item_in_hands)
		{
			if (item_in_hands.IsInherited(FreezeAnvil_Kit) || item_in_hands.IsInherited(FreezeFurnace_Kit) || item_in_hands.IsInherited(FreezeGrindingStone_Kit))
			{
